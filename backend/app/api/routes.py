...
def get_ai_portfolio_leaderboard(db: Session = Depends(get_db)):
    ...
    # AUDIT 2026-06-19 bughunter #3: usa il SINGLETON di pnl_tracker invece
    # di creare new YahooFetcher per request. Unifica cache + percorso fetch.
    from app.services.ai_portfolio import pnl_tracker as _pnl
    spy_price = None
    spy_pct = None
    try:
        from datetime import date as _d, timedelta as _td
        s = _pnl._fetcher().fetch(
            "SPY", start_date=_d.today() - _td(days=15), end_date=_d.today(),
        )
